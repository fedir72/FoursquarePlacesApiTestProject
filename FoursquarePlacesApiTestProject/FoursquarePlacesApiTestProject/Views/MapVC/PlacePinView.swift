...
class PlacePinView: MKAnnotationView {
    var photoUrl: URL? {
        didSet {
            imageView.sd_setImage(with: self.photoUrl,placeholderImage: UIImage(systemName: "umbrella"))
        }
    }
     ...
     // guard let place = newValue as? PlaceMapPin else { return }
      ...
      calloutOffset = CGPoint(x: -5, y: 5)
        let mapsButton: UIButton = {
           let btn = UIButton(frame: CGRect(origin: CGPoint.zero, size: CGSize(width: 35, height: 40)))
